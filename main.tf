...
resource "github_branch" "onion_kitchen" {
  branch     = "kitchen"
  repository = github_repository.onion.name
}

resource "github_branch_default" "onion_kitchen" {
  branch     = github_branch.onion_kitchen.branch
  repository = github_repository.onion.name
}

resource "github_branch_protection" "onion_kitchen" {
  pattern           = github_branch_default.onion_kitchen.branch
  repository_id       = github_repository.onion.id
  required_status_checks {
    strict   = true
    contexts = ["continuous-integration"]
  }
